...
✅ **已上生产**：2026-08-24 23:29 UTC 切到 `kiro-rs:06a17f3`，实际停机 17 秒（`docker compose up -d` 先停旧再起新，还要等凭据加载与健康检查——比初估的 1-2 秒长，这个差值要记住）。

生产实测：`GET /api/admin/stats/rate` 返回 120 桶；真实流量下入口与上游确实分离（一次请求故障转移 3 个凭据 = 入口 1 / 上游 3），`retryAmplification` 在分钟翻转后精确算出 3.0；概览页面板字样已在下发的 `overview-page-*.js` 里逐条命中。
