...
pub fn profile_presets() -> Vec<ProfilePreset> {
    vec![
        ProfilePreset {
            ...
            label: "优先级",
            summary: "按优先级排队，从最优先的号开始用，它不可用了才换下一个",
            ...
            caveat: "选号是粘滞的：流量会一直压在最优先那一个号上，直到它出错才换。                     单号并发受上游限制，所以这个模式的吞吐≈单账号吞吐，不适合冲量。                     好处是消耗集中、账好对，且额度守卫仍在（95% 自动降级）。",
        },
        ProfilePreset {
            profile: SchedulingProfile::Conserve,
            label: "权重",
            summary: "按余额加权铺开，余额多的多接流量，各号见底时间趋于一致",
            demote_threshold_pct: 90.0,
            demote_to: 70,
            min_top_tier: 2,
            throughput_burn_below_pct: 80.0,
            throughput_reserve_at_pct: 95.0,
            ...
            throttle_cooldown_secs: 1800,
            spill_on_rate_limit: false,
            caveat: "日常默认档。避免「某个号先饿死→可用池缩小→并发挤在剩下的号上」。                     限流时不换号、冷却 30 分钟，所以失败率会比扛 TPM 高一些。",
        },
        ProfilePreset {
            profile: SchedulingProfile::HighConcurrency,
            label: "扛 TPM",
            summary: "全部号并列跑满，429 立刻换号，只有击穿额度的才退到后排接溢出",
            demote_threshold_pct: 95.0,
            demote_to: 70,
            min_top_tier: 2,
            ...
            caveat: "冲峰值用，不适合常态开着 —— 它不增加月度额度总量，只会更快烧完。                     开之前先看吞吐预估里的断供缺口。不给任何号减速，一直用到击穿。",
        },
    ]
}
