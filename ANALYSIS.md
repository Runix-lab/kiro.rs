...
| 要 RPM / TPM | RPM 只是限流闸门不是指标（默认关闭、重启清零、口径是上游跳数）；TPM 完全不存在；聚合器最细只到小时 | **已建采集层**：分钟环 + `/api/admin/stats/rate` + 概览页面板，入口/上游双口径。未上生产（§6） |
...
**结论先行（分析当时）：RPM 存在，但它是一个「限流闸门」，不是一个「可观测指标」。TPM 完全不存在。** 面板上没有任何现成数据可读，必须新建采集层。

> **已实现（2026-08-24）**：采集层与面板都已落地，本节以下内容保留为设计依据与实现前的现状记录。交付物见 `RPM-TPM-ANALYSIS.md` §0。
>
> 实现时撞到两条设计阶段没预见的约束，都值得记住：
> 1. **上游计数不能挂 `TraceSink`** —— `emit_attempt` 在 sink 为 `None` 时早退，挂那儿等于关掉 traces.db 上游 RPM 一起归零，而 traces.db 是用户可关的。做法是把 `emit_attempt` 从关联函数改成方法（31 个调用点），计数放在早退**之前**。
> 2. **环必须只有一个实例** —— 由 anthropic 路由建立并塞进 provider，主程序再 `provider.rate_ring()` 取回给 Admin。若两侧各建一个环，各自只数到一半流量。
>
> 另外：采集层未注入时 `/api/admin/stats/rate` 回 **503 而不是一堆 0**，否则调用方分不清「没装采集层」与「真的没流量」。
